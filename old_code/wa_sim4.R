...
se_RSC(data[ss_items], ml_ss$w, parms[ss_items, ], ml_ss$theta1, ml_ss$theta2, method = "ML", obs = F)
...
stan_ll <- stan(file = RSC_logit, data = ll_data, iter = 2000, chains = 4)
bayes_ll <- summary(stan_ll , pars = "u")$summary

# long group test, short individual test
ls_data <- format_stan_data(data, parms, ls_items)
stan_ls <- stan(file = RSC_logit, data = ls_data, iter = 2000, chains = 4)
...
stan_ss <- stan(file = RSC_logit, data = ss_data, iter = 2000, chains = 4)
